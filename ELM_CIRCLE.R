...
Xin<-as.matrix(Circle$x)
Xin<-cbind(1,Xin)
Yin<-as.matrix(as.numeric(Circle$classes))
...
image(MZ)
contour(seqx1x2,seqx1x2,MZ,nlevels=1,xlim=c(-1,1),ylim = c(-1,1),xlab="X",ylab="Y",main=paste("ELM com dados Circle e p=",p))
par(new=TRUE)
